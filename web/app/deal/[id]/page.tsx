        ...
        <CardContent className="space-y-6 pt-6">
          ...
          <div className="space-y-1 bg-slate-800/30 p-4 rounded-lg border border-slate-800">
            <InfoRow
              ...
              isShareLink // Bật chế độ Share Link
            />
            <InfoRow
              label="Seller"
              value={fields.seller}
              isSuiName
              isCopyable // Thêm nút copy ví cho Seller
            />
            <InfoRow
              label="Buyer"
              value={fields.buyer}
              ...
              isSuiName
              isCopyable // Thêm nút copy ví cho Buyer
            />
          </div>
          ...
          <div className="pt-2">
            ...
            {fields.status === 0 && (
              <div className="flex flex-col gap-3">
                ...
                {!account && (
                  <div className="text-center text-sm text-slate-400 p-2 bg-slate-800 rounded mb-2">
                    👇 Vui lòng kết nối ví để thực hiện nạp tiền
                  </div>
                )}

                <Button
                  className="w-full bg-blue-600 hover:bg-blue-500 font-bold h-12 text-base shadow-lg shadow-blue-900/20"
                  onClick={() =>
            ...
            {fields.status === 1 && (
              <div className="space-y-4">
                <div className="p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-lg text-yellow-200 text-center text-sm flex flex-col gap-1">
                ...
                <Button
                  className="w-full bg-green-600 hover:bg-green-500 font-bold h-12 text-lg shadow-lg shadow-green-900/20"
                  onClick={() =>
                ...
                <div className="pt-6 mt-6 border-t border-slate-800/50">
                  <Button
                    ...
                    onClick={() => {
                      if (
                        confirm(
                          "⚠️ CẢNH BÁO: Bạn có chắc chắn muốn hủy kèo này không?"
                        )
                      ) {
                        cancelDeal(dealId, () =>
                          setTimeout(() => window.location.reload(), 1000)
                  ...
                  >
                    🚫 Hủy kèo & Hoàn tiền (Emergency Cancel)
                  </Button>
                </div>
              )}
    ...
    <div className="flex justify-between items-center py-3 border-b border-slate-700/50 last:border-0 hover:bg-slate-800/20 px-2 rounded transition-colors -mx-2">
      ...
      <div className="flex items-center gap-3">
        ...
        {(isShareLink || (isCopyable && value)) && (
          <CopyButton
            text={isShareLink ? undefined : value} // Nếu là share link thì để tự động lấy URL
            isUrl={isShareLink}
